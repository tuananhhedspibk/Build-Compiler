...
enum OpCode {
	OP_LA,   // Load Address:    t := t + 1; s[t] := base(p) + q;								// OK
	OP_LV,   // Load Value:      t := t + 1; s[t] := s[base(p) + q];							// OK
	OP_LC,   // load Constant    t := t + 1; s[t] := q;											// OK
	OP_LI,   // Load Indirect    s[t] := s[s[t]];												// OK
	OP_INT,  // Increment t      t := t + q;													// OK
	OP_DCT,  // Decrement t      t := t - q;													// OK
	OP_J,    // Jump             pc := q;														// OK
	OP_FJ,   // False Jump       if s[t] = 0 then pc := q; t := t - 1;							// OK
	OP_HL,   // Halt             Halt															// OK
	...
	OP_AD,   // Add              t := t-1;  s[t] := s[t] + s[t+1];									// OK
	OP_SB,   // Substract        t := t-1;  s[t] := s[t] - s[t+1];									// OK
	OP_ML,   // Multiple         t := t-1;  s[t] := s[t] * s[t+1];									// OK
	OP_DV,   // Divide           t := t-1;  s[t] := s[t] / s[t+1];									// OK
	OP_NEG,  // Negative         s[t] := - s[t];													// OK
	OP_CV,   // Copy Top         s[t+1] := s[t]; t := t + 1;										// OK
	OP_EQ,   // Equal            t := t - 1;  if s[t] = s[t+1] then s[t] := 1 else s[t] := 0;		// OK
	OP_NE,   // Not Equal        t := t - 1;  if s[t] != s[t+1] then s[t] := 1 else s[t] := 0;		// OK
	OP_GT,   // Greater          t := t - 1;  if s[t] > s[t+1] then s[t] := 1 else s[t] := 0;		// OK
	OP_LT,   // Less             t := t - 1;  if s[t] < s[t+1] then s[t] := 1 else s[t] := 0;		// OK
	OP_GE,   // Greater or Equal t := t - 1;  if s[t] >= s[t+1] then s[t] := 1 else s[t] := 0;		// OK
	OP_LE,   // Less or Equal    t := t - 1;  if s[t] >= s[t+1] then s[t] := 1 else s[t] := 0;		// OK

	OP_BP    // Break point. Just for debugging														// OK
};
